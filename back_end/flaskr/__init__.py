...
from models import setup_db,MainMenus,CardData
...
def create_app(test_config=None):
  ...
  @app.route('/mainmenus')
  def get_all_mainmenus():

    """Get all mainmenus with id and title
       or status code 500 due to server error
    """
    
    try:
      menu_query = MainMenus.query.order_by(MainMenus.id).all()
      
      formated_menu = [menu.format() for menu in menu_query]
      return jsonify({
        'success':True,
        'mainmenus':formated_menu,
        'total_menus': len(formated_menu)
      }),200
    except:
      abort(500)
  ...
  def get_all_cards():
    ...
    """Get all cards data with id , name, image
       or status code 500 due to server error
    """
    
    try:
      card_query = CardData.query.order_by(CardData.id).all()
      
      formated_card = [card.format() for card in card_query]
      return jsonify({
        'success':True,
        'carddata':formated_card,
        'total_cards': len(formated_card)
      }),200
    except:
      abort(500)
